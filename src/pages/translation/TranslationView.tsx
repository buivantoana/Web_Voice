...
const TranslationView = ({
  ...
  const handleTranslate = async() => {
    setLoading(true)
    const translateService = selectedTranslation === "chatgpt" ? "GPT" : "Deepseek";
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", mt: "-30px" }}>
            <Box width={"45%"}>
              <VisualSlider title={t("origin")} onChange={setOriginalVolume} value={originalVolume} />
            </Box>
            <Box width={"45%"}>
              <VisualSlider title={t("translated")} onChange={setTtsVolume} value={ttsVolume} />
            </Box>
          </Box>
          <Typography sx={{ my: "20px", fontWeight: "500", width: "45%" }}>
            {t("sub")}
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", mt: "-30px" }}>
            ...
            <Box width={"45%"}>
              <VisualSlider title={t("size")} onChange={setSizeSubtitle} value={sizeSubtitle} min={8} max={24} />
            </Box>
          </Box>
...
function CustomSwitch({ onChange, checked }: { onChange: (value: boolean) => void; checked: boolean }) {
  const handleToggle = () => {
    onChange(!checked);
  };
...
function SubtitleEditor({ onSubtitlesChange,subtitles }: { onSubtitlesChange: (subs: Subtitle[]) => void ,subtitles:any}) {
  ...
  const handleAdd = () => {
    ...
    const newSubs = [...subs, { id: newId, start: "", end: "", text: "" }];
    setSubs(newSubs);
    onSubtitlesChange(newSubs);
  };
